...
from wallet.schemas import (WalletCreateSerializer, WalletUpdateSerializer, WalletUpdateParticularSerializer,
                            Wallet as WalletSerializer)
...
async def create_wallet(form_data: WalletCreateSerializer,
...
async def read_wallets(skip: int = 0,
                       limit: int = 10,
                       current_user: User = Depends(get_current_user),
                       crud: WalletCRUD = Depends()):
...
async def update_wallet(wallet_id: int,
...
async def update_wallet_particular(wallet_id: int,
